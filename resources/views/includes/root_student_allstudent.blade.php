<div class="bg-white py-3 px-6 ml-4 mr-8 text-gray-600 my-5 rounded">
    <h1 class="text-lg font-semibold py-4 w-full">All Student</h1>
    @if(count($student) > 0)
            ...
            <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
                <div class="overflow-hidden">
                    <table class="w-full"">
                        ...
                        <tbody>
                            @foreach($student as $worker)
                                <tr class="divide-y divide-gray-300 border-b-2">
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        {{ $worker->StudentfullName($worker->id) }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        <span class="flex">
                                            <form action="{{ route('student-edit-step-1', $worker->id) }}" method="GET">
                                                @csrf 
                                                <input type="submit" value="EDIT" class="edit-btn">
                                            </form>
                                            &nbsp;&nbsp;
                                            <form action="{{ route('student-delete', $worker->id) }}" method="POST">
                                                @csrf 
                                                @method('DELETE')
    ...
    @else
        <h1 class="text-lg font-semibold py-4 w-full">No Student Found</h1>
    @endif
</div>